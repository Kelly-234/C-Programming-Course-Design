...
/*********************************************
FUNCTION:inputadmin
DESCRIPTION：输入手机号、身份证号、新增数据等无需隐藏的信息
INPUT:id(input string),x1,y1,charnum(maxlength),color(input box background color),size(font size(16 or 32))
RETURN:无
IMPROVE:只能输入数字、小写字母、大写X
***********************************************/
...
void inputadmin(char* id, int x1, int y1, int charnum, int color, int size)
{
	...
	bar(x1,y1,x1+20+18*charnum,y1+size+2);
	...
	line(x1 + 10 - 5*flag, y1 + 9 - 9*flag + flag, x1 + 10 - 5*flag, y1 + 25 - 9*flag + flag);
	while(bioskey(1))
	{
		bioskey(0);
	}
	while (1)
	{
		...
		if (i < charnum)
		{
				...
				&& t != 033)//Esc
			{
				if (t != '\b')
				{
					if(!(t>='0'&&t<='9'||t=='X'||t>='a'&&t<='z'))
						continue;
					*(id + i) = t;
					*(id + i + 1) = '\0';
					bar(x1 + 9 + i * 18 - 5*flag, y1, x1 + 11 + i * 18 - 5*flag, y1 + size+2); //遮盖光标
					outtextxy(x1 + 10 + i * 18, y1+3*flag - 5*flag, id + i);//输出t字符(16)
					i++;
					line(x1 + 10 + i * 18 - 5*flag, y1 + 9 - 9*flag + flag, x1 + 10 + i * 18 - 5*flag, y1 + 25 - 9*flag + flag);
				}
				else if (t == '\b' && i > 0)//退格键
				{
					bar(x1 + 9 + i * 18 - 5*flag, y1, x1 + 11 + i * 18 - 5*flag, y1 + size+2);//遮盖光标
					bar(x1 - 13 + i * 18 - 5*flag, y1, x1 + 6 + i * 18 - 5*flag, y1 + size+2);//遮盖文字
					i--;//减少一个字数
					line(x1 + 10 + i * 18 - 5*flag, y1 + 9 - 9*flag + flag, x1 + 10 + i * 18 - 5*flag, y1 + 25 - 9*flag + flag);//绘制光标
					*(id + i) = '\0';
					*(id + i + 1) = '\0';
				}
			}
			...
			{
				setfillstyle(SOLID_FILL, color);
				bar(x1 + 9 + i * 18 - 5*flag, y1 + 7 - 7*flag, x1 + 11 + i * 18 - 5*flag, y1 + 27 - 7*flag);//遮盖光标
				break;
			}
		}
		else if (i >= charnum)
...
DESCRIPTION：输入密码等需要隐藏的信息
INPUT:id(input string),x1,y1,charnum(maxlength),color(input box background color)
RETURN:无
IMPROVE:只能输入数字和字母
***********************************************/
...
void inputpassword(char* id, int x1, int y1, int charnum, int color)
{
	...
	clrmous(MouseX, MouseY);
	setfillstyle(SOLID_FILL, color);
	bar(x1,y1,x1+20+18*charnum,y1+34);
	setlinestyle(SOLID_LINE, 0, NORM_WIDTH);
	settextstyle(TRIPLEX_FONT, HORIZ_DIR, 1);
	settextjustify(LEFT_TEXT, TOP_TEXT);
	setcolor(DARKGRAY);
	line(x1 + 10, y1 + 9, x1 + 10, y1 + 25);
	while(bioskey(1))
	{
		bioskey(0);
	}
		...
		if (i < charnum)
		{
				...
				&& t != 033)//033表示Esc
			{
				if (t != '\b')
				{
					...
					*(id + i) = t;
					*(id + i + 1) = '\0';
					bar(x1 + 8 + i * 18, y1 + 7, x1 + 12 + i * 18, y1 + 27);//遮盖光标
					outtextxy(x1 + 8 + i * 18, y1 + 9, "*");//输出*字符
					i++;
					line(x1 + 10 + i * 18, y1 + 9, x1 + 10 + i * 18, y1 + 25);
				}
				else if (t == '\b' && i > 0)//退格键
				{
					bar(x1 + 8 + i * 18, y1 + 7, x1 + 12 + i * 18, y1 + 27);//遮盖光标
					bar(x1 - 10 + i * 18, y1 + 7, x1 + 8 + i * 18, y1 + 27);//遮盖文字
					i--;//减少一个字数
					line(x1 + 10 + i * 18, y1 + 9, x1 + 10 + i * 18, y1 + 25);//绘制光标
					*(id + i) = '\0';
					*(id + i + 1) = '\0';
				}
			}
